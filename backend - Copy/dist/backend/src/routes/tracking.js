 ...
 */
exports.trackingRouter.post('/scan', (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    try {
        ...
        if (isUnit) {
            ...
            const saResult = await client.query('SELECT sa.*, i.id as item_id FROM storage_assignments sa JOIN items i ON sa.item_id = i.id WHERE sa.unit_code = $1 AND sa.checked_out_at IS NULL', [scan_code]);
            const maResult = await client.query('SELECT ma.*, m.code as machine_code FROM machine_assignments ma JOIN machines m ON ma.machine_id = m.id WHERE ma.unit_code = $1 AND ma.removed_at IS NULL', [scan_code]);
        ...
        if (location_code.startsWith('M/')) {
            ...
            if (sourceType === 'machine' && currentAssignment.machine_id === machine.id) {
                ...
                await client.query(`INSERT INTO activity_log (id, item_id, action, from_location, to_location, performed_by, notes)
                 VALUES ($1, $2, 'note_added', $3, $4, $5, $6)`, [(0, uuid_1.v4)(), itemId, 'note_added', location_code, location_code, performed_by, `Status toggled to ${nextStatus}`]);
            ...
            if (!isUnit) {
                ...
                if (itemResult.rows.length === 0)
                    throw new Error(`Item template ${scan_code} not found. Create item first.`);
                ...
                unitCode = (0, trackingUnits_1.getNextTrackingUnitCode)(item.item_code, existingCodesResult.rows.map(r => r.unit_code));
            }
            else {
            ...
            const newMaId = (0, uuid_1.v4)();
            await client.query(`INSERT INTO machine_assignments (id, item_id, machine_id, unit_code, status, quantity, assigned_at, assigned_by)
             VALUES ($1, $2, $3, $4, $5, $6, NOW(), $7)`, [newMaId, itemId, machine.id, unitCode, 'queued', 1, performed_by]);
            await client.query(`INSERT INTO activity_log (id, item_id, action, from_location, to_location, performed_by, notes)
             VALUES ($1, $2, 'move', $3, $4, $5, $6)`, [(0, uuid_1.v4)(), itemId, sourceType === 'shelf' ? 'Storage' : 'New', location_code, performed_by, `Started production phase`]);
            await client.query('COMMIT');
    ...
    `, [location_code]).catch(() => {
        ...
        if (shelfResult.rows.length > 0) {
            ...
            if (!isUnit)
                throw new Error("Cannot move a template to storage. Check into a machine first to generate a Unit ID.");
            // Checkout from old
            ...
            await client.query(`INSERT INTO storage_assignments (id, item_id, shelf_slot_id, unit_code, quantity, checked_in_at, checked_in_by)
             VALUES ($1, $2, $3, $4, $5, NOW(), $6)`, [newSaId, itemId, shelf.id, unitCode, 1, performed_by]);
        ...
        if (location_code === 'EXIT') {
            ...
            await client.query(`INSERT INTO activity_log (id, item_id, action, from_location, to_location, performed_by, notes)
             VALUES ($1, $2, 'check_out', $3, 'SHIPPED', $4, $5)`, [(0, uuid_1.v4)(), itemId, location_code, performed_by, `Final dispatch via scanner`]);
            await client.query('COMMIT');
