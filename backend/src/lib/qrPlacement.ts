...
import { extractQrToken } from './qrLinks';
...
export async function resolveItemForQr(client: PoolClient, qrCode: string): Promise<ItemRecord | null> {
  ...
  const qrMatch = await client.query<ItemRecord>(`
    SELECT i.id, i.item_code, i.name, c.name AS customer_name, i.customer_id, i.order_number, i.created_at, i.type, i.quantity
    FROM qr_entities qe
  ...
  const result = await client.query(`
    SELECT ss.id AS shelf_slot_id, r.id AS rack_id, r.code AS rack_code, r.label AS rack_label, ss.row_number, ss.column_number,
      ...
      COALESCE((
        ...
        JOIN items i2 ON i2.id = sa2.item_id
        JOIN shelf_slots ss2 ON ss2.id = sa2.shelf_slot_id
        WHERE sa2.checked_out_at IS NULL
          AND ss2.rack_id = r.id
          AND i2.customer_id = $6
      ), 0) AS same_customer_count
    FROM shelf_slots ss
    JOIN racks r ON r.id = ss.rack_id
    WHERE ss.current_count < ss.capacity
    ORDER BY rack_priority ASC, rack_type_penalty ASC, same_customer_count DESC, display_distance ASC, cell_distance ASC, r.display_order ASC, ss.row_number ASC, ss.column_number ASC
    LIMIT 1
  `, [targetRackId, targetDisplayOrder, targetRow, targetColumn, targetRackType, customerId || null]);
